...
Int_t StPicoDstMaker::MakeRead() {
  ...
  Int_t read = mChain->GetEntry(mEventCounter++);
  mPicoDst->set(this);
...
//-----------------------------------------------------------------------
bool StPicoDstMaker::filterMtdEvent()
{
  ///==============================
  /// MTD event filtering
  /// single-muon: keep events with at least one triggering hit that is matched to a track
  /// dimuon-hft: keep events with at least two triggering hits that are matched to tracks
  bool filter = false;
  StPicoEvent *event = mPicoDst->event();
  if(!event) return filter;
  ...
  if(event->isSingleMuon())
    {
      if(nMthTrigHits<1) 
	{
	  LOG_DEBUG << "Reject singl-muon events with " << nMthTrigHits << " matched hits!" << endm;
	  filter = true;
	}
    }
  else if(event->isDiMuonHFT() && !event->isEMuon() && nMthTrigHits<2) 
    {
      // if a dimuon-hft event overlaps with a emu trigger,
      // this event should not be rejected if there are less than 2
      // matched MTD hits, since the emu analysis only needs 1 hit
      LOG_DEBUG << "Reject dimuon-hft events with " << nMthTrigHits << " matched hits!" << endm;
      filter = true;
    }
  else
    {
      LOG_DEBUG << "Keep events with " << nMthTrigHits << " matched hits!" << endm;
    }
...
void StPicoDstMaker::fillTracks() {
  ...
  for(int i=0;i<nGlobals;i++) {
    ...
    Int_t flowFlag = 0; // Not relevant for only muons
...
//-----------------------------------------------------------------------
void StPicoDstMaker::fillEventPlane() 
{
  StPicoEvent *event = mPicoDst->event();
  if(event && event->isMtdTrig())
    {
      int counter = mPicoArrays[picoEventPlane]->GetEntries();
      new((*(mPicoArrays[picoEventPlane]))[counter]) StPicoEventPlane(*mMuDst);
    }
}
